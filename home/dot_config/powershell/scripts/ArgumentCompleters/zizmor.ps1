...
Register-ArgumentCompleter -Native -CommandName 'zizmor' -ScriptBlock {
    ...
    $completions = @(switch ($command) {
        'zizmor' {
            ...
            [CompletionResult]::new('--gh-token', '--gh-token', [CompletionResultType]::ParameterName, 'The GitHub API token to use [env: GH_TOKEN or GITHUB_TOKEN or ZIZMOR_GITHUB_TOKEN]')
            [CompletionResult]::new('--github-token', '--github-token', [CompletionResultType]::ParameterName, 'This is an alias for `--gh-token` / `GH_TOKEN`')
            [CompletionResult]::new('--zizmor-github-token', '--zizmor-github-token', [CompletionResultType]::ParameterName, 'This is an alias for `--gh-token` / `GH_TOKEN` / `--github-token` / `GITHUB_TOKEN`')
